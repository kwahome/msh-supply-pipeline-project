<?php
	...
	$sites_desc_string = $retrieve[2];
	$sites_desc_retrieve = explode(",", $sites_desc_string);
	$sites_number_string = $sites_desc_retrieve[0];
	$sites_number_string_retrieve = explode("=", $sites_number_string);
	$number_of_sites = $sites_number_string_retrieve[1];

	$sites_array = array();
	for($count =1; $count<=$number_of_sites; $count++)
	{
		$sites_array[] = $sites_desc_retrieve[$count];
	}

	$site_classification = $retrieve[3];
    ...
    if(mysqli_num_rows($check_program_result)>0)
    {
    	...
    	if(mysqli_num_rows($check_name_result)>0)
    	{
    		...
    		for($count =1; $count<=$number_of_sites; $count++)
			{
				$site_id = $sites_desc_retrieve[$count];
    	...
    	// Program not confirmed. Doesn't exist
    	else
    	{
